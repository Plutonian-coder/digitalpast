...
export function FeatureCard({ feature, className, ...props }: FeatureCardPorps) {
    // Use a stable seed for consistent SSR/client rendering
    const p = React.useMemo(() => genRandomPattern(0), []);
...
function genRandomPattern(seed: number = 0, length?: number): number[][] {
    length = length ?? 5;
    return Array.from({ length }, (_, i) => [
        Math.floor(seededRandom(seed + i * 2) * 4) + 7, // random x between 7 and 10
        Math.floor(seededRandom(seed + i * 2 + 1) * 6) + 1, // random y between 1 and 6
    ]);
}
