    ...
    return (
        <div className="flex-1 overflow-y-auto p-6 lg:p-10 bg-background">
            ...
            <div className="flex flex-col gap-4 mb-8">
                <nav className="flex items-center text-sm text-muted-foreground flex-wrap gap-2">
                    ...
                    <ChevronRight className="w-4 h-4" />
                    <span className="hover:text-primary transition-colors cursor-pointer" onClick={onBack}>{level}</span>
                    <ChevronRight className="w-4 h-4" />
                    <span className="font-bold text-foreground bg-primary/10 px-2 py-0.5 rounded">{courseCode}</span>
                </nav>

                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-2xl lg:text-3xl font-bold text-foreground mb-2">{courseTitle}</h1>
                        <p className="text-muted-foreground text-sm">Showing {questions.length} past questions found</p>
                    </div>
                    <button
                        onClick={onBack}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
                {questions.length === 0 ? (
                    <div className="col-span-full text-center py-20 text-muted-foreground">
                        No past questions found for this course.
                    </div>
                ) : questions.map((q) => (
                    <div key={q.id} className="bg-card border border-border rounded-[2rem] p-8 group hover:bg-muted/50 transition-all relative">
                        <div className="flex justify-between items-start mb-6">
                            <div className="flex gap-2">
                                <span className="px-3 py-1 bg-primary text-[10px] font-bold rounded-lg text-primary-foreground">
                                    {q.course_code}
                                </span>
